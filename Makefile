...
ifneq (${current_ocaml_version},${ocaml_version})
	...
	@cp _build/default/src/bin_node/main.exe cuprum-node
	@cp _build/default/src/bin_validation/main_validator.exe cuprum-validator
	@cp _build/default/src/bin_client/main_client.exe cuprum-client
	@cp _build/default/src/bin_client/main_admin.exe cuprum-admin-client
	@cp _build/default/src/bin_signer/main_signer.exe cuprum-signer
	@cp _build/default/src/bin_codec/codec.exe cuprum-codec
	@cp _build/default/src/lib_protocol_compiler/main_native.exe cuprum-protocol-compiler
	@for p in $(active_protocol_directories) ; do \
	   cp _build/default/src/proto_$$p/bin_baker/main_baker_$$p.exe cuprum-baker-`echo $$p | tr -- _ -` ; \
	   cp _build/default/src/proto_$$p/bin_endorser/main_endorser_$$p.exe cuprum-endorser-`echo $$p | tr -- _ -` ; \
	   cp _build/default/src/proto_$$p/bin_accuser/main_accuser_$$p.exe cuprum-accuser-`echo $$p | tr -- _ -` ; \
...
doc-html: all
	@dune build @doc
	@./cuprum-client -protocol ProtoALphaALphaALphaALphaALphaALphaALphaALphaDdp3zK man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-client.html
	@./cuprum-admin-client man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-admin-client.html
	@./cuprum-signer man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-signer.html
	@./cuprum-baker-alpha man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-baker-alpha.html
	@./cuprum-endorser-alpha man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-endorser-alpha.html
	@./cuprum-accuser-alpha man -verbosity 3 -format html | sed "s#${HOME}#\$$HOME#g" > docs/api/cuprum-accuser-alpha.html
...
build-sandbox:
	...
	@cp _build/default/src/bin_sandbox/main.exe cuprum-sandbox
...
clean: coverage-clean
	...
	@-rm -f \
		...
		cuprum-admin-client \
		cuprum-codec \
		cuprum-protocol-compiler \
		cuprum-sandbox \
	  $(foreach p, $(active_protocol_versions), cuprum-baker-$(p) cuprum-endorser-$(p) cuprum-accuser-$(p)) \
	...
	@-${MAKE} -C docs clean
	@-rm -f docs/api/tezos-{baker,endorser,accuser}-alpha.html docs/api/tezos-{admin-,}client.html docs/api/cuprum-signer.html
